...
namespace ImGui
{
	...
	void ColorPickerBox(const char* picker_idname, float col_ct[], float col_t[], float col_ct_invis[], float col_t_invis[], bool alpha = true)
	{
		...
		if (ImGui::BeginPopup(picker_idname))
		{
			...
			if (g_Options.esp_player_chams_type == 4)
			{
				const char* button_name1 = switch_color_vis ? "Wire" : "Model";
				if (ImGui::Button(button_name1, ImVec2(-1, 0)))
					switch_color_vis = !switch_color_vis;
			}
			else if (g_Options.esp_player_chams_type == 5)
			{
				const char* button_name1 = switch_color_vis ? "Terrorists" : "Counter-Terrorists";
				if (ImGui::Button(button_name1, ImVec2(-1, 0)))
					switch_color_vis = !switch_color_vis;
			}
			else
...
namespace GladiatorMenu
{
	...
	void GUI_Init(HWND window, IDirect3DDevice9 *pDevice)
	{
		...
		ImVec4 col_main = ImColor(32, 32, 48); //ImColor(9, 82, 128);
		ImVec4 col_back = ImColor(16, 16, 24);
		ImVec4 col_area = ImColor(16, 16, 25);
		ImVec4 col_theme = ImColor(32, 32, 48);
		ImVec4 col_title = ImColor(16, 16, 26);

		style.Colors[ImGuiCol_Text] = ImVec4(1.00f, 1.00f, 1.00f, 1.00f);
		style.Colors[ImGuiCol_TextDisabled] = ImVec4(0.60f, 0.60f, 0.60f, 1.00f);
		style.Colors[ImGuiCol_WindowBg] = ImVec4(0.16f, 0.16f, 0.24f, 1.00f);
		...
		style.Colors[ImGuiCol_FrameBg] = ImVec4(0.32f, 0.32f, 0.48f, 1.00f);
		style.Colors[ImGuiCol_FrameBgHovered] = ImVec4(0.32f, 0.32f, 0.48f, 1.00f);
		style.Colors[ImGuiCol_FrameBgActive] = ImVec4(0.32f, 0.32f, 0.48f, 1.00f);
		style.Colors[ImGuiCol_TitleBg] = ImVec4(0.16f, 0.16f, 0.25f, 1.00f);
		style.Colors[ImGuiCol_TitleBgCollapsed] = ImVec4(0.16f, 0.16f, 0.25f, 1.00f);
		style.Colors[ImGuiCol_TitleBgActive] = ImVec4(0.16f, 0.16f, 0.25f, 1.00f);
		style.Colors[ImGuiCol_MenuBarBg] = ImVec4(0.20f, 0.20f, 0.30f, 1.00f);
		style.Colors[ImGuiCol_ScrollbarBg] = ImVec4(0.16f, 0.16f, 0.24f, 1.00f);
		style.Colors[ImGuiCol_ScrollbarGrab] = ImVec4(0.08f, 0.08f, 0.012f, 1.00f);
		style.Colors[ImGuiCol_ScrollbarGrabHovered] = ImVec4(0.32f, 0.32f, 0.48f, 1.00f);
		style.Colors[ImGuiCol_ScrollbarGrabActive] = ImVec4(0.32f, 0.32f, 0.42f, 1.00f);
		...
		style.Colors[ImGuiCol_SliderGrabActive] = ImVec4(0.16f, 0.16f, 0.25f, 1.00f);
		style.Colors[ImGuiCol_Button] = ImVec4(0.16f, 0.16f, 0.25f, 1.00f);
		style.Colors[ImGuiCol_ButtonHovered] = ImVec4(0.16f, 0.16f, 0.25f, 1.00f);
		style.Colors[ImGuiCol_ButtonActive] = ImVec4(0.16f, 0.16f, 0.25f, 1.00f);
		style.Colors[ImGuiCol_Header] = ImVec4(0.16f, 0.16f, 0.30f, 0.90f);
		style.Colors[ImGuiCol_HeaderHovered] = ImVec4(0.16f, 0.16f, 0.24f, 1.00f);
		style.Colors[ImGuiCol_HeaderActive] = ImVec4(0.24f, 0.16f, 0.16f, 1.00f);
		...
		style.Colors[ImGuiCol_CloseButton] = ImVec4(0.85, 0.10, 0.10, 0.75f); 
		style.Colors[ImGuiCol_CloseButtonHovered] = ImVec4(0.85, 0.10, 0.10, 0.65f);
	...
	void mainWindow()
	{
		...
		ImGui::SetNextWindowSize(ImVec2(860, 585), ImGuiSetCond_FirstUseEver);

		// I don't know if you want this but this disables all that tab to change value bullshit.
		//ImGui::PushAllowKeyboardFocus(false);

		ImGui::PushFont(title_font);
		if (ImGui::Begin("Exodia by Smiley", &menuOpen, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_ShowBorders | ImGuiWindowFlags_OnlyDragByTitleBar))
		{
			ImGui::BeginGroup();

			static int selected_Tab = 0;
			static const char* items[5] = { "Legitbot", "Ragebot", "HVH", "Visual", "Misc"};
			ImGui::SelectTabs(&selected_Tab, items, 5);
		...
		{
			if (ImGui::Begin("Exodia_[DEBUG PARAMETERS]", &g_Options.debug_window, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_ShowBorders))
			{
	...
	void ragebotTab()
	{
		ImGui::Columns(1, NULL, false);
		{
			ImGui::BeginChild("RAGEBOTCHILD", ImVec2(0, 0), true);
			{
				...
				//	ImGui::Checkbox("Enable Aimstep##Rage", &g_Options.rage_Aimstep);
					/*if (ImGui::IsItemHovered())
				...
				ImGui::NextColumn();
				{
					...
					{
					ImGui::SliderFloat("Stop Value", &g_Options.rage_autostop_value, 1, 10);
					}
					//ImGui::Checkbox("Auto Baim if slowwalk##Rage", &g_Options.rage_baimifslowwalk);
					ImGui::Checkbox("Auto Body-Aim##Rage", &g_Options.rage_autobaim);
					if (g_Options.rage_autobaim)
						ImGui::SliderInt("Body-Aim After Shots##Rage", &g_Options.rage_baim_after_x_shots, 0, 5);
				}
				ImGui::Separator();
				ImGui::Columns(1);

				//ImGui::Combo("Weapon##Box", &g_Options.WeaponMisc, opt_WeaponMisc, 8);
			//	if (g_Options.WeaponMisc == 0)
				{
					ImGui::Columns(1, NULL, true);
					{

						ImGui::Checkbox("Prioritize Selected Hitbox##Scout", &g_Options.rage_prioritize);
						ImGui::Combo("Hitbox Priority##Scout", &g_Options.rage_hitbox, opt_AimHitboxSpot, 4);
						ImGui::Checkbox("Multipoint##Scout", &g_Options.rage_multipoint);
						if (g_Options.rage_multipoint)
						{
						ImGui::SliderFloat("Pointscale##Scout", &g_Options.rage_pointscale, 0.0f, 1.0f);
						}
						ImGui::SliderFloat("Min. damage##Scout", &g_Options.rage_mindmg, 0.f, 120.f, "%.2f");
						ImGui::SliderFloat("Hitchance %##Scout", &g_Options.rage_hitchance_amount, 0.f, 100.0f, "%.2f");
		...
		ImGui::Columns(2, NULL, false);
		{
			ImGui::BeginChild("COL1", ImVec2(0, 0), true);
			{
				...
				ImGui::Columns(1);
				{
					...
					ImGui::Checkbox("Ignore Enemies##ESP", &g_Options.esp_only_enemies);
					//ImGui::Checkbox("Ignore Local##ESP", &g_Options.esp_only_local);
				...
				//	ImGui::Checkbox("SpreadCrosshair##ESP", &g_Options.esp_player_spreadcross);
					ImGui::Checkbox("Skelet##ESP", &g_Options.esp_player_skelet);
					ImGui::Checkbox("Visualize Backtrack##ESP", &g_Options.esp_backtracked_player_skelet);
					...
					ImGui::Combo("Type##ESP", &g_Options.esp_lagcompensated_hitboxes_type, std::vector<std::string>{ "Actual Hit", "Compensated Lag", "Both", "Both ~ Dark" });
					//ImGui::SliderFloat("Duration##ESP", &g_Options.esp_LagCompDuration, 1.f, 6.f);
					ImGui::Checkbox("Angle Lines##ESP", &g_Options.esp_player_anglelines);
					ImGui::Checkbox("Chams##ESP", &g_Options.esp_player_chams);
					ImGui::Combo("Chams Type##ESP", &g_Options.esp_player_chams_type, opt_Chams, 5);
					if (g_Options.esp_player_chams_type == 4)
					{ 
					ImGui::ColorPickerBox("##Picker_chams_players", g_Options.esp_player_chams_color_ct, g_Options.esp_player_chams_color_t, g_Options.esp_player_chams_color_ct_wire, g_Options.esp_player_chams_color_t_wire, false);
					}
					else if (g_Options.esp_player_chams_type == 5)
					{
					ImGui::ColorPickerBox("##Picker_chams_players", g_Options.esp_player_chams_color_ct, g_Options.esp_player_chams_color_t, g_Options.esp_player_chams_color_ct_colored, g_Options.esp_player_chams_color_t_colored, false);
					}
					else
					{ 
					...
					ImGui::Checkbox("Transparency##ESP", &g_Options.visuals_transcheck);
					if (g_Options.visuals_transcheck)
					{
						ImGui::SliderFloat("", &g_Options.visuals_trans, 0, 1.f);
					}
				}
		...
		ImGui::NextColumn();
		{
			ImGui::BeginChild("COL2", ImVec2(0, 0), true);
			{
				...
				ImGui::Columns(1, NULL, true);
				{
					...
					ImGui::Combo("HitSounds##Others", &g_Options.visuals_others_hitmarkersound, opt_Hitsounds, 1);
					//ImGui::Checkbox("AA Key Arrows", &g_Options.esp_AAArrows);
				...
				ImGui::Columns(1, NULL, true);
				{
					...
					ImGui::Checkbox("Zoomfix", &g_Options.removals_Zoomfix);
				}
				...
				ImGui::Checkbox("Offscreen ESP##ESP", &g_Options.esp_offscreen);
				...
				ImGui::Checkbox("World Modification##ESP", &g_Options.misc_worldmodcheck);
				if (g_Options.misc_worldmodcheck)
				{
					ImGui::Combo("World type##ESP", &g_Options.misc_worldmod, opt_worldmod, 4);
				}
		...
		ImGui::Columns(2, NULL, false);
		{
			ImGui::BeginChild("COL1", ImVec2(0, 0), true);
			{
				...
				ImGui::Columns(1, NULL, true);
				{
					...
					ImGui::Checkbox("Anti AFK##Other", &g_Options.misc_afk);
					...
					ImGui::Checkbox("Exodia Clan-Tag##Other", &g_Options.misc_animated_clantag);
					...
					ImGui::Checkbox("Instantduck", &g_Options.misc_Instantduck);
					ImGui::Checkbox("Slowwalk##Other", &g_Options.misc_Slowwalk);
					ImGui::KeyBindButton(&g_Options.misc_Slowwalk_bind);
				//	ImGui::SliderFloat("Speed", &g_Options.misc_Slowwalk_Amount, 1, 200);
					//ImGui::Text("TickExploit");
					//ImGui::KeyBindButton(&g_Options.misc_TimeExploit);
					//ImGui::SliderFloat("Ticks", &g_Options.misc_Ticks, 1.f, 100.f);
			...
			ImGui::BeginChild("COL2", ImVec2(0, 0), true);
			{
				...
				ImGui::Columns(1, NULL, true);
				{
					...
					if (ImGui::Button("Save##Config"))
					{
						...
						{
							std::string fPath = std::string(Global::my_documents_folder) + "\\Exodia\\" + configItems[configItemCurrent] + ".uwu";
					...
					{
						g_EngineClient->ExecuteClientCmd("quit");
					}
					...
					{
						std::string fPath = std::string(Global::my_documents_folder) + "\\Exodia\\" + fName + ".uwu";
						Config::Get().SaveConfig(fPath);
				...
				ImGui::Separator();
				{
				//	ImGui::Checkbox("Head Spoofer##Mods", &g_Options.misc_Headspoofer);
				//	if (g_Options.misc_Headspoofer)
				//	{
				//		ImGui::Checkbox("Only Guns", &g_Options.misc_goodweapon);
				//	}

					ImGui::Checkbox("AlwaysInventoryOn##Mods", &g_Options.misc_Inv);
					ImGui::Checkbox("Gravity##Mods", &g_Options.misc_Gravity);
					if (g_Options.misc_Gravity)
					{
						ImGui::SliderFloat("GravitySlider", &g_Options.misc_GravitySlider, 0, 1000);
					}
					ImGui::Checkbox("Pushscale##Mods", &g_Options.misc_Pushscale);
					if (g_Options.misc_Pushscale)
					{
						ImGui::SliderFloat("PushscaleSlider", &g_Options.misc_PushscaleSlider, 0, 1000);
					}
					ImGui::Checkbox("Latency To Shoot##Mods", &g_Options.misc_Latency);
					if (g_Options.misc_Latency)
					{
						ImGui::SliderFloat("Latency Slider", &g_Options.misc_LatencySlider, 0, 0.200);
					}
					ImGui::Checkbox("Slidewalk##Mods", &g_Options.misc_Slidewalk);
					//ImGui::Checkbox("Flags##Mods", &g_Options.misc_CheatInfo);
		...
		ImGui::Columns(1, NULL, true);
		{
			ImGui::BeginChild("COL1", ImVec2(0, 0), true);
			{
				...
				ImGui::NextColumn();
				{
					...
					ImGui::NewLine();
					ImGui::Combo("##AAY", &g_Options.hvh_antiaim_y, opt_AAyaw, 6);
					ImGui::NewLine();
					ImGui::Combo("##FAAY", &g_Options.hvh_antiaim_y_fake, opt_AAfakeyaw, 7);
					if (g_Options.hvh_antiaim_y_fake == 5)
					{
						ImGui::Text("CorruptionSlider");
						ImGui::SliderFloat("Curruption", &g_Options.CorruptionSlider, -145.f, 145.f);
					}
					ImGui::NewLine();
					ImGui::Text("Manualleft Key");
					ImGui::KeyBindButton(&g_Options.hvh_Manualleftkey);
					ImGui::NewLine();
					ImGui::Text("Manualleft Right");
					ImGui::KeyBindButton(&g_Options.hvh_Manualrightkey);
					ImGui::NewLine();
					ImGui::Text("Manualleft Back");
					ImGui::KeyBindButton(&g_Options.hvh_Manualbackkey);
					ImGui::NewLine();
					ImGui::Text("Positiv Jitter");
					ImGui::SliderFloat("+", &g_Options.hvh_PositivJitter, 0, 90.f);
					ImGui::NewLine();
					ImGui::Text("Negativ Jitter");
					ImGui::SliderFloat("-", &g_Options.hvh_NegativJitter, 0, -90.f);
					ImGui::NewLine();
